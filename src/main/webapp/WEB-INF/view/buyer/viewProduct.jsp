...
<%@page import="com.einfo.SmartInventoryManagement.Model.Product"%>
...
<body>
	...
	<% List<Product> buyer = (List<Product>) request.getAttribute("buyerspage");
%>
	<h1>Product Table</h1>
	...
	<script>
		function sortdes()
		{
			window.location.href = "/ViewProductData?offset=${currPage}&limit=3&sortBy=desc";
		}
		function sort()
		{
			window.location.href = "/ViewProductData?offset=${currPage}&limit=3";
		}
		
		
	</script>
	
		<!-- <form action="/ViewProductData" method="get">
    <input type="text" name="search" placeholder="Search by product name">
    <button type="submit">Search</button>
</form> -->
	<table border="1">
		<tr>
			<th>Name <button id="sort" onclick="sort()">asc</button>  <button id="sortdes" onclick="sortdes()">des</button></th>
			<th>Description</th>
			<th>Quantity</th>
			<th>Threshold</th>
			<th>Price</th>
		...
		if(buyer!=null)
		{
		for (Product buyerproduct : buyer) {
			
		%>
		<tr>
			<td><%=buyerproduct.getProductname()%></td>
			<td><%=buyerproduct.getProductdecs()%></td>
			<td><%=buyerproduct.getProductquantity()%></td>
			<td><%=buyerproduct.getProductthreshold()%></td>
			<td><%=buyerproduct.getProductprice()%></td>
		</tr>
		<%
		}
		}
	...
	<% int totalCount = (Integer)request.getAttribute("totalData"); %>
	<% int limit = 3; %>
	...
	<%if(request.getAttribute("currPage")!=null){ %>
	  ...
	  <% if (currPage > 0) { %>
			<a href="/ViewProductData?offset=${currPage -1}&limit=3">Previous</a> 
		<%} %>
	<%} %>
	

<a href="/ViewProductData?offset=${currPage + 1}&limit=3">Next</a>
