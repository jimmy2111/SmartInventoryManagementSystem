...
<%@page import="com.einfo.SmartInventoryManagement.Model.Sender"%>
...
<body>
	...
	<%
	List<Sender> sender = (List<Sender>) request.getAttribute("sellerdetail");
	%>
	...
	<table border="1">
		...
		<%
		if (sender != null) {
			for (Sender senderDetail : sender) {
		%>
		<tr>
			<td><%=senderDetail.getSender_name()%></td>
			<td><%=senderDetail.getSender_Shop_name()%></td>
			<td><%=senderDetail.getSender_address()%></td>
			<td><%=senderDetail.getSender_contact()%></td>
		</tr>
		<%
		}
		}
	...
	<%if(request.getAttribute("currPage")!=null){ %>
	  ...
	  <% if (currPage > 0) { %>
			<a href="/sellerDetail?offset=${currPage -1}&limit=3">Previous</a> 
		<%} %>
	<%} %>
	<a href="/sellerDetail?offset=${currPage + 1}&limit=3">Next</a>
